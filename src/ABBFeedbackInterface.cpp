...
namespace open_abb_driver {
        ...
        ABBFeedbackInterface::ABBFeedbackInterface( const std::string& ip, int port, size_t bufferSize ) : outgoing( bufferSize ) {
		...
		// Read all information from the tcp/ip socket
			...
			boost::split( splits, buff, boost::is_any_of( "#" ) );
				...
				Feedback msg;
					...
					char date[2000];
					char time[2000];
                                        sscanf(partialBuffer,"%*d %s %s %*f %lf %lf %lf %lf %lf %lf %lf", date, time, &cmsg.x, &cmsg.y, &cmsg.z, &cmsg.qw, &cmsg.qx, &cmsg.qy, &cmsg.qz);
					...
					char date[2000];
					char time[2000];
                                        sscanf(partialBuffer,"%*d %s %s %*f %lf %lf %lf %lf %lf %lf", date, time, &jmsg.joints[0], &jmsg.joints[1], &jmsg.joints[2], &jmsg.joints[3], &jmsg.joints[4], &jmsg.joints[5] );
